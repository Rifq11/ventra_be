...
async function seedDatabase() {
    ...
    const executeWithRetry = async (statement: string, maxRetries = 3) => {
    ...
    try {
        ...
        try {
            await connection.query(
                "ALTER TABLE ventra_produk MODIFY COLUMN Gambar LONGBLOB NULL"
            );
            console.log('🔧 Ensured ventra_produk.Gambar is LONGBLOB');
        } catch (alterErr: any) {
            console.warn(
                '⚠️  Could not alter ventra_produk.Gambar to LONGBLOB (might already be correct):',
                alterErr.message
            );
        }
        ...
        const insertVentraProdukWithBlob = async (statement: string): Promise<boolean> => {
            try {
                connection = await ensureConnection();
                ...
                await connection.execute(
                    'INSERT INTO ventra_produk (id, Nama_Brg, Bahan, harga_jual, Kategori, Gambar) VALUES (?, ?, ?, ?, ?, ?)',
                    [id, namaBrg, bahan, hargaJual, kategori, gambarBuffer]
                );
        ...
        const splitMultiRowInsert = (statement: string): string[] => {
            // Match INSERT INTO `table` (...) VALUES (...), (...), ...
            const match = statement.match(/^(INSERT INTO `[^`]+` \(`[^)]+`\) VALUES\s*)(.+)$/i);
            if (!match) return [statement]; // Can't parse, return as-is
